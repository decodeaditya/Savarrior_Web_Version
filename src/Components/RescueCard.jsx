  ...
  return (
    ...
    <Box sx={{
      ...
      <Box sx={{ p: '6px 14px' }}>
        ...
        <Typography sx={{display:"flex",mt:"10px",alignItems:"center",fontSize:"18px"}}><LocationOnRounded sx={{color:colors.primaryicons, borderRadius: "50%", p: "5px",mr:"7px", border: `2px solid ${colors.primary}` }} />{props.rescue?.location[0].address}</Typography>
      </Box>
    ...
    <Modal
      ...
      >
       <Box sx={{width:{md:"70%",xs:'90%'},height:{md:"70%",xs:"90%"},borderRadius:"5px"}}><iframe width="100%" title={props.rescue?.id} height="100%" frameBorder="0" marginHeight="0" marginWidth="0" src={`https://maps.google.com/maps?q=${props.rescue?.location[1].coords[0].latitude},${props.rescue?.location[1].coords[1].longitude}&z=15&output=embed`}></iframe></Box>
      </Modal>
    </React.Fragment>
  )
}
